    ...
    def __init__(self,
        ...
        if (meshes is None):
            ...
            for k_meshes in range(len(meshes)):
                ...
                point_data = self.meshes_readers[-1].GetOutput().GetPointData()
                n_arrays = point_data.GetNumberOfArrays()
                if (n_arrays >= 1):
                    array_name = point_data.GetArray(0).GetName()
                    self.meshes_readers[-1].GetOutput().GetPointData().SetActiveVectors(array_name)
    ...
    def update_index(self, index=0):
        ...
        if (self.has_meshes):
            for k_meshes in range(len(self.meshes)):
                ...
                point_data = self.meshes_readers[k_meshes].GetOutput().GetPointData()
                n_arrays = point_data.GetNumberOfArrays()
                if (n_arrays >= 1):
                    array_name = point_data.GetArray(0).GetName()
                    self.meshes_readers[k_meshes].GetOutput().GetPointData().SetActiveVectors(array_name)
